...
def _create_dummy_catalog(base_path: Path):
    """Generates a dummy file-based catalog if none exists."""
    ...
    for prod in dummy_products:
        ...
        d.text((10,10), prod["name"], fill=(255,255,255))
        img.save(prod_dir / "img_1.jpg")
...
def load_multimodal_catalog():
    ...
    for folder in catalog_path.iterdir():
        if folder.is_dir():
            ...
            try:
                ...
                # FUSION STRATEGY: Average text embedding and all image embeddings
    ...
    logger.info(f"Multimodal catalog loaded: {len(_catalog_items)} products indexed.")

# Load catalog immediately upon service initialization
...
def search_multimodal_catalog(query_text: str = None, query_image_bytes: bytes = None, top_k: int = 3) -> dict:
    ...
    logger.info(f"Executing multimodal search. Text provided: {bool(query_text)}. Image provided: {bool(query_image_bytes)}")
